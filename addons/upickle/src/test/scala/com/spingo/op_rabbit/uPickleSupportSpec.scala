package com.spingo.op_rabbit

import org.scalatest.FunSpec
import org.scalatest.Matchers

case class Thing(a: Int)

object Thing {
  import upickle.default.{ReadWriter => RW, macroRW}
  implicit val rw: RW[Thing] = macroRW
}

class upickleSupportSpec extends FunSpec with Matchers {
  describe("upickleSupport.ujsonDefault") {
    import upickleSupport.ujsonDefault._
    val u = implicitly[RabbitUnmarshaller[Thing]]
    val m = implicitly[RabbitMarshaller[Thing]]

    it("deserializes the provided content") {
      u.unmarshall("""{"a": 5}""".getBytes, Some("application/json"), Some("UTF-8")) should be (Thing(5))
    }

    it("interprets no encoding / no contentType as json / UTF8") {
      u.unmarshall("""{"a": 5}""".getBytes, None, None) should be (Thing(5))
    }

    it("rejects wrong encoding") {
      a [MismatchedContentType] should be thrownBy {
        u.unmarshall("""{"a": 5}""".getBytes, Some("text"), Some("UTF-8"))
      }
    }

    it("serializes the provided content") {
      val body = m.marshall(Thing(5))
      new String(body) should be ("""{"a":5}""")
    }

    it("provides the appropriate content headers") {
      val properties = m.setProperties().build
      properties.getContentType should be ("application/json")
      properties.getContentEncoding should be ("UTF-8")
    }
  }

  describe("upickleSupport.upackDefault") {
    import upickleSupport.upackDefault._
    val u = implicitly[RabbitUnmarshaller[Thing]]
    val m = implicitly[RabbitMarshaller[Thing]]


    it("deserializes the provided content") {
      val bytes = upickle.default.writeBinary(Thing(5))
      u.unmarshall(bytes, Some("application/msgpack"), None) should be (Thing(5))
    }

    it("interprets no encoding / no contentType as msgpack") {
      val bytes = upickle.default.writeBinary(Thing(5))
      u.unmarshall(bytes, None, None) should be (Thing(5))
    }

    it("rejects wrong encoding") {
      a [MismatchedContentType] should be thrownBy {
        u.unmarshall("""{"a": 5}""".getBytes, Some("text/json"), Some("UTF-8"))
      }
    }

    it("serializes the provided content") {
      val body = m.marshall(Thing(5))
      upickle.default.readBinary[Thing](body) should be (Thing(5))
    }

    it("provides the appropriate content headers") {
      val properties = m.setProperties().build
      properties.getContentType should be ("application/msgpack")
      properties.getContentEncoding should be (null)
    }
  }

}
